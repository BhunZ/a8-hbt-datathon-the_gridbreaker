...
Strategy
--------
 - log1p targets for RMSE stability.
 - Calendar + Fourier + Tet indicator + static lags (730/1095/1460) + recursive lag_365.
 - Rolling same-(month,day) seasonal means + trailing 365d level features.
 - LightGBM + CatBoost blend (0.55 / 0.45). Recency-weighted sample weights.
 - Validation on 2021-07-01 -> 2022-12-31 (horizon-matched 548 days).
 - Recursive predict: fill lag_365 in 2024 with own 2023 predictions.
"""
...
def sample_weight(dates: pd.Series, half_life_years: float = 2.0) -> np.ndarray:
    anchor = dates.max()
...
def run_val(target, feats):
    ...
    # recent-only subset (2019+) with uniform weights
    ...
    # Optimize blend weights — but keep LGBM at >= 0.15 for variance reduction.
    best = (1e18, None)
...
print("\n=== Validation metrics (549 days, 2021-07-01 to 2022-12-31) ===")
...
# 6. Recursive predict over test window (fill lag_365 in 2024) ------------
